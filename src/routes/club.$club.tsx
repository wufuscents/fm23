...
function ClubPage() {
  ...
  const rankedTrophies = useMemo(() => [...players].sort((a, b) => Number((b as any).trophies || 0) - Number((a as any).trophies || 0)), [players])
  ...
  const totals = useMemo(() => ({
    ...
    trophies: players.reduce((sum, p) => sum + Number((p as any).trophies || 0), 0),
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="overflow-hidden rounded-3xl border bg-slate-900/80 p-6 sm:p-10" style={{ borderColor: `${color}88`, background: `radial-gradient(circle at 85% 10%, ${color}30, transparent 35%), rgba(15,23,42,.9)` }}>
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            <div className="flex h-24 w-24 items-center justify-center rounded-2xl border bg-slate-950/70 p-3" style={{ borderColor: `${color}66` }}>
              {logo ? <img src={storageUrl(logo)} alt={club} className="h-full w-full object-contain" /> : <span className="font-heading text-xl font-black" style={{ color }}>{club.split(/\s+/).slice(0, 3).map((x) => x[0]).join('').toUpperCase()}</span>}
            </div>
            <div><div className="font-mono text-[10px] uppercase tracking-[0.3em]" style={{ color }}>CLUB ARCHIVE</div><h1 className="mt-1 font-heading text-4xl font-black uppercase tracking-wider text-white sm:text-6xl">{club}</h1><p className="mt-2 font-mono text-xs text-slate-500">{players.length} archived players connected to this club.</p></div>
          </div>
          <div className="mt-8 grid grid-cols-2 gap-2 sm:grid-cols-4">{Object.entries({ Players: players.length, 'Club Apps': totals.apps, 'Club Goals': totals.goals, Trophies: totals.trophies }).map(([k, v]) => <div key={k} className="rounded-xl border border-white/10 bg-slate-950/60 p-3"><div className="font-mono text-xl font-black text-white">{Number(v).toLocaleString()}</div><div className="text-[9px] uppercase tracking-widest text-slate-600">{k}</div></div>)}</div>
        </div>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
          ...
          {nations.length ? <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">{nations.map(([name, info]) => <Link key={name} to="/nation/$nation" params={{ nation: name }} className="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-950/60 p-3 transition-colors hover:border-slate-600"><div className="h-8 w-11 flex items-center justify-center">{info.flag ? <img src={info.flag} alt="" className="max-h-6 max-w-9 object-contain" /> : null}</div><span className="flex-1 truncate font-heading font-bold uppercase text-white">{name}</span><span className="font-mono text-[9px] text-slate-500">{info.count}</span></Link>)}</div> : <Empty />}
        </section>
      </div>
    </div>
  )
...
function LegacySection({ title, color, players }: { title: string; color: string; players: Player[] }) {
  return <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><h2 className="font-heading text-2xl font-black uppercase" style={{ color }}>{title}</h2><div className="mt-4 space-y-2">{players.map((p, i) => <PlayerRow key={p.id} player={p} rank={i + 1} />)}{!players.length && <Empty />}</div></section>
}
