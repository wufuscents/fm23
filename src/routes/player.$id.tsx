...
function PlayerProfilePage() {
  ...
  const nationColor =
    TEAM_COLORS[playerNation] ||
    (isFemale ? '#ec4899' : isMale ? '#3b82f6' : '#64748b')
  const statusAccent = isLegend ? '#fbbf24' : isIcon ? '#cbd5e1' : nationColor

  const totalCareerApps = useMemo(() => {
    return sortedPlayerCareer.reduce(
      (sum: number, c: any) => sum + (c.apps || 0),
      0,
    )
  }, [sortedPlayerCareer])
  ...
  const totalCareerGoals = useMemo(() => {
    return sortedPlayerCareer.reduce(
      (sum: number, c: any) => sum + (c.goals || 0),
      0,
    )
  }, [sortedPlayerCareer])
  ...
  const totalTrophiesCount = useMemo(() => {
    ...
    return teamTrophies.reduce(
      (sum: number, a: any) => sum + (a.amount || 1),
      0,
    )
  }, [player, teamTrophies])
  ...
  const totalAwardsCount = useMemo(() => {
    ...
    return individualAwards.reduce(
      (sum: number, a: any) => sum + (a.amount || 1),
      0,
    )
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {/* Back navigation — preserves the exact Directory state */}
        <button
          type="button"
              ...
              <div className="flex flex-col items-center lg:items-stretch gap-3">
                ...
                >
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${nationColor}35, transparent 45%, ${statusAccent}22)`,
                    }}
                  />
                  ...
                  {playerImage ? (
                    ...
                    <span className="relative z-10 text-slate-500 font-mono text-xs">
                      NO IMAGE
                    </span>
                  )}
                ...
                <div className="grid grid-cols-2 gap-2 font-mono text-center">
                  <div className="rounded-lg border border-white/10 bg-slate-950/60 px-2 py-2">
                    ...
                    <div
                      className="mt-0.5 text-xs font-bold uppercase"
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="rounded-xl border border-white/10 bg-slate-950/55 p-3 flex items-center gap-3">
                    <div className="h-11 w-11 flex-shrink-0 rounded-lg border border-white/10 bg-slate-900/80 flex items-center justify-center p-2">
                      {latestClubLogo ? (
                        <img
                          src={storageUrl(latestClubLogo)}
                          alt=""
                          className="h-full w-full object-contain"
                        />
                      ) : (
                        <span className="font-heading text-[10px] font-bold text-slate-500">
                          CLUB
                        </span>
                      )}
                    </div>
                ...
                <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-7 gap-2 font-mono text-center">
                  {[
                    ...
                    >
                      ...
                      <div className="mt-0.5 text-[8px] uppercase tracking-widest text-slate-600">
                        {label}
                      </div>

                      <div
                        className="mx-auto mt-2 h-0.5 w-8 rounded-full transition-all duration-300 group-hover/stat:w-12"
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 font-mono text-xs">
            {(awardTab === 'team' ? teamTrophies : individualAwards).map(
              (item: any) => (
                <div
                  key={item.id}
